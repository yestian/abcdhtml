...
class TopbarEye extends Component{
    render(){
        let active=null;
        if(!!this.props.eyeStatus===true){
            active='bem-TopBar_Body_PreviewButton-active';
        }else{
            active='';
        }
        return(
            <div data-automation-id="preview-mode-button" className={`bem-TopBar_Body_Button bem-TopBar_Body_PreviewButton ${active}`} onClick={(e)=>this.props.onClick(e,!this.props.eyeStatus)}
